...
// {rk0, rk1, rk2, rk3} <- vs2
# define VSM4_KEYGEN(rk3, rk2, rk1, rk0, imm, rk7, rk6, rk5, rk4) \
    { \
    const uint8_t round = (imm) & 0x7; \
    uint32_t B = (rk1) ^ (rk2) ^ (rk3) ^ zvksed_ck[4 * round]; \
    uint32_t S = ZVKSED_SUB_BYTES(B); \
    (rk4) = ZVKSED_ROUND_KEY((rk0), S); \
    B = (rk2) ^ (rk3) ^ (rk4) ^ zvksed_ck[4 * round + 1]; \
    S = ZVKSED_SUB_BYTES(B); \
    (rk5) = ZVKSED_ROUND_KEY((rk1), S); \
    B = (rk3) ^ (rk4) ^ (rk5) ^ zvksed_ck[4 * round + 2]; \
    S = ZVKSED_SUB_BYTES(B); \
    (rk6) = ZVKSED_ROUND_KEY((rk2), S); \
    B = (rk4) ^ (rk5) ^ (rk6) ^ zvksed_ck[4 * round + 3]; \
    S = ZVKSED_SUB_BYTES(B); \
    (rk7) = ZVKSED_ROUND_KEY((rk3), S); \
    };

// vd = {x0, x1,x2, x3} <- vd
// {rk0, rk1, rk2, rk3} <- vs2
#define VSM4_ROUND(x3, x2, x1, x0, rk3, rk2, rk1, rk0, x7, x6, x5, x4) \
{ \
    uint32_t B; \
    uint32_t S; \
    B = (x1) ^ (x2) ^ (x3) ^ (rk0); \
    S = ZVKSED_SUB_BYTES(B); \
    (x4) = ZVKSED_ROUND((x0), S); \
    B = (x2) ^ (x3) ^ (x4) ^ (rk1); \
    S = ZVKSED_SUB_BYTES(B); \
    (x5) = ZVKSED_ROUND((x1), S); \
    B = (x3) ^ (x4) ^ (x5) ^ (rk2); \
    S = ZVKSED_SUB_BYTES(B); \
    (x6) = ZVKSED_ROUND((x2), S); \
    B = (x4) ^ (x5) ^ (x6) ^ (rk3); \
    S = ZVKSED_SUB_BYTES(B); \
    (x7) = ZVKSED_ROUND((x3), S); \
};
